...
# Defining colors to use for each line
fg.OR = Style(RgbFg(250,169,32))
fg.GR = Style(RgbFg(144,246,116))
fg.RD = Style(RgbFg(252, 83, 83))
fg.YL = Style(RgbFg(252, 214, 111))
fg.SV = Style(RgbFg(213, 208, 208))
fg.BL = Style(RgbFg(138, 220, 254))
...
################################ ORANGE LINE ################################
            if lineCode == "OR":
                ...
                print(f"Train ID: {fg.OR}{train_id}{rs.all}")
                print(f"Line: {fg.OR}{lineCode}{rs.all}")
                
                # Print station name if the circuit ID matches.
                if circuitID in CIRCUITBYNAME:
                    stationName = CIRCUITBYNAME[circuitID]
                    print(f"Station: {fg.OR}{stationName}{rs.all}")
                # If there's no match, the train is deemed not at a station and will return "En route..."
                else:
                    print(f"Station: {fg.OR}En route...{rs.all}")
                
                print(f"Circuit ID: {fg.OR}{circuitID}{rs.all}")
                
                # Print direction information 
                if direction == 1:
                    print(f"Direction: {fg.OR}Northbound/Eastbound{rs.all}")
                elif direction == 2:
                    print(f"Direction: {fg.OR}Southbound/Westbound{rs.all}")
                else:
                    print(f"{fg.OR}Directional data not found{rs.all}.")

################################ BLUE LINE ################################
            elif lineCode == "BL":

                # Print information for all trains.
                print(f"Train ID: {fg.BL}{train_id}{rs.all}")
                print(f"Line: {fg.BL}{lineCode}{rs.all}")
                
                # Print station name if the circuit ID matches.
                if circuitID in CIRCUITBYNAME:
                    stationName = CIRCUITBYNAME[circuitID]
                    print(f"Station: {fg.BL}{stationName}{rs.all}")
                # If there's no match, the train is deemed not at a station and will return "En route..."
                else:
                    print(f"Station: {fg.BL}En route...{rs.all}")
                
                print(f"Circuit ID: {fg.BL}{circuitID}{rs.all}")
                
                # Print direction information 
                if direction == 1:
                    print(f"Direction: {fg.BL}Northbound/Eastbound{rs.all}")
                elif direction == 2:
                    print(f"Direction: {fg.BL}Southbound/Westbound{rs.all}")
                else:
                    print(f"{fg.BL}Directional data not found{rs.all}.")

################################ RED LINE ################################
            elif lineCode == "RD":

                # Print information for all trains.
                print(f"Train ID: {fg.RD}{train_id}{rs.all}")
                print(f"Line: {fg.RD}{lineCode}{rs.all}")
                
                # Print station name if the circuit ID matches.
                if circuitID in CIRCUITBYNAME:
                    stationName = CIRCUITBYNAME[circuitID]
                    print(f"Station: {fg.RD}{stationName}{rs.all}")
                # If there's no match, the train is deemed not at a station and will return "En route..."
                else:
                    print(f"Station: {fg.RD}En route...{rs.all}")
                
                print(f"Circuit ID: {fg.RD}{circuitID}{rs.all}")
                
                # Print direction information 
                if direction == 1:
                    print(f"Direction: {fg.RD}Northbound/Eastbound{rs.all}")
                elif direction == 2:
                    print(f"Direction: {fg.RD}Southbound/Westbound{rs.all}")
                else:
                    print(f"{fg.RD}Directional data not found{rs.all}.")
            
################################ YELLOW LINE ################################
            elif lineCode == "YL":

                # Print information for all trains.
                print(f"Train ID: {fg.YL}{train_id}{rs.all}")
                print(f"Line: {fg.YL}{lineCode}{rs.all}")
                
                # Print station name if the circuit ID matches.
                if circuitID in CIRCUITBYNAME:
                    stationName = CIRCUITBYNAME[circuitID]
                    print(f"Station: {fg.YL}{stationName}{rs.all}")
                # If there's no match, the train is deemed not at a station and will return "En route..."
                else:
                    print(f"Station: {fg.YL}En route...{rs.all}")
                
                print(f"Circuit ID: {fg.YL}{circuitID}{rs.all}")
                
                # Print direction information 
                if direction == 1:
                    print(f"Direction: {fg.YL}Northbound/Eastbound{rs.all}")
                elif direction == 2:
                    print(f"Direction: {fg.YL}Southbound/Westbound{rs.all}")
                else:
                    print(f"{fg.YL}Directional data not found{rs.all}.")
            
################################ SILVER LINE ################################
            elif lineCode == "SV":

                # Print information for all trains.
                print(f"Train ID: {fg.SV}{train_id}{rs.all}")
                print(f"Line: {fg.SV}{lineCode}{rs.all}")
                
                # Print station name if the circuit ID matches.
                if circuitID in CIRCUITBYNAME:
                    stationName = CIRCUITBYNAME[circuitID]
                    print(f"Station: {fg.SV}{stationName}{rs.all}")
                # If there's no match, the train is deemed not at a station and will return "En route..."
                else:
                    print(f"Station: {fg.SV}En route...{rs.all}")
                
                print(f"Circuit ID: {fg.SV}{circuitID}{rs.all}")
                
                # Print direction information 
                if direction == 1:
                    print(f"Direction: {fg.SV}Northbound/Eastbound{rs.all}")
                elif direction == 2:
                    print(f"Direction: {fg.SV}Southbound/Westbound{rs.all}")
                else:
                    print(f"{fg.SV}Directional data not found{rs.all}.")

################################ GREEN LINE ################################
            elif lineCode == "GR":

                # Print information for all trains.
                print(f"Train ID: {fg.GR}{train_id}{rs.all}")
                print(f"Line: {fg.GR}{lineCode}{rs.all}")
                
                # Print station name if the circuit ID matches.
                if circuitID in CIRCUITBYNAME:
                    stationName = CIRCUITBYNAME[circuitID]
                    print(f"Station: {fg.GR}{stationName}{rs.all}")
                # If there's no match, the train is deemed not at a station and will return "En route..."
                else:
                    print(f"Station: {fg.GR}En route...{rs.all}")
                
                print(f"Circuit ID: {fg.GR}{circuitID}{rs.all}")
                
                # Print direction information 
                if direction == 1:
                    print(f"Direction: {fg.GR}Northbound/Eastbound{rs.all}")
                elif direction == 2:
                    print(f"Direction: {fg.GR}Southbound/Westbound{rs.all}")
                else:
                    print(f"{fg.GR}Directional data not found{rs.all}.")
...
################################ END LINE COLOR PRINTING ################################
    ...
    # If there's an error code, print failure and return the code.
        ...
        print()
        print(f"{fg.RD}Failed to retrieve data. Error:{rs.all} {response.status_code}")
