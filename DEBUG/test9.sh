...
wget -q -O "$WMATADIR/$T_NAME" "$TRACKERURL"
...
for ((i=countdown; i>0; i--)); do
    ...
    # Sleep for a shorter time to allow for more frequent checks
    sleep 0.5

    # Check if the user pressed 's'
    if read -t 0.1 -n 1 key && [[ $key == "s" ]]; then
        printf "\nCountdown skipped!"
        break
    fi
done
...
if sudo systemctl start $SVC_NAME; then
    ...
    sudo journalctl -u $SVC_NAME --since "10 minutes ago"
fi

# Run the tracker in the foreground
printf "\n${PINK}Running WMATA RailTracker in the foreground...$RESETCOLOR\n"
python3 "$WMATADIR/$T_NAME"
