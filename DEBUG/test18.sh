...
# Use a subshell to allow background process handling
(
for ((i=countdown; i>0; i--)); do
    echo -ne "Starting in $ORANGE$i$RESETCOLOR seconds...\r"
    sleep 1 &  # Start sleep in the background
    read -t 1 -n 1 key  # Wait for 1 second for a single key press
    if [[ $key == "s" ]]; then
        printf "\nCountdown skipped!\n"
        kill $!  # Kill the background sleep process
        break
    fi
done
)

# Ensure the countdown message is cleared
printf "\n${PINK}Starting the service...$RESETCOLOR\n"
sleep 2

# Start the Python script as the service
python3 -u "$WMATADIR/$T_NAME" &

# If you want to see output, you can redirect it to a log file
# python3 -u "$WMATADIR/$T_NAME" >> "$WMATADIR/output.log" 2>&1
